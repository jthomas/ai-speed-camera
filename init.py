...
parser.add_argument('--video', help='the video file to process', type=argparse.FileType('r'))
...
if args.export_to_csv is not None:
    ...
    with open(args.export_to_csv, mode='w', newline='') as csv_file:
        ...
        to_csv.writerow(["Car Detected #", "Entrance Time (s)", "Exit Time (s)", "Speed (km/h)"])
        for idx, car in enumerate(cars_frame_lookup):
            to_csv.writerow([idx, car['entrance_time'], car['exit_time'], car['car_speed']])
...
if args.video is not None and args.output is not None:
    ...
    logging.info("Missing video and output parameters - skipping source video annotation.")
